...
def classify_letter(bgr):
    """
    Detect one glyph in `bgr` and return
        (tag, poly)
    where  tag  ∈ {'A','B','C'} or None
           poly = 4×2 int32 array of the glyph bounding box (or None)
    """
    # -- 1. dark-pixel mask -------------------------------------------------
    ...
    bw     = cv2.morphologyEx(bw, cv2.MORPH_OPEN, KERNEL, 1)  # remove pinholes
    ...
    # largest surviving blob
    c   = max(cands, key=lambda t: t[0])[1]

    # -- 3. warp glyph to 128×128 ROI --------------------------------------
    ...
    # -- 4. binarise & hole analysis ---------------------------------------
    _, lmask = cv2.threshold(cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY),
                             0, 255,
                             cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

    cntsL, hier = cv2.findContours(lmask, cv2.RETR_CCOMP,
                                   cv2.CHAIN_APPROX_SIMPLE)
    if hier is None:
        return None, box.astype(int)
    ...
    for i, h in enumerate(hier[0]):
        if h[3] != -1:                       # child contour = hole
            if cv2.contourArea(cntsL[i]) >= HOLE_MIN:
                holes += 1

    # -- 5. decide with adaptive solidity ----------------------------------
    hull_area = cv2.contourArea(cv2.convexHull(c)) + 1e-8
    solidity  = cv2.contourArea(c) / hull_area

    if   holes >= 2:     tag = 'B'                # two large holes
    elif holes == 1:     tag = 'A'                # one hole
    else:                tag = 'C' if solidity < 0.60 else None
                         # C = 0 hole *and* strongly concave
